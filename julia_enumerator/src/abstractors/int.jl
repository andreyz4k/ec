...
function reverse_plus(value, calculated_arguments)
    ...
    if calculated_arguments[1] !== missing && calculated_arguments[1] !== any_object
        if calculated_arguments[2] !== missing && calculated_arguments[2] !== any_object
            if value - calculated_arguments[1] != calculated_arguments[2]
                error("Calculated arguments $calculated_arguments don't add up to value $value")
            else
                return [calculated_arguments[1], calculated_arguments[2]]
            end
        else
            return [calculated_arguments[1], value - calculated_arguments[1]]
        end
    elseif calculated_arguments[2] !== missing && calculated_arguments[2] !== any_object
        return [value - calculated_arguments[2], calculated_arguments[2]]
    end
...
function reverse_minus(value, calculated_arguments)
    ...
    if calculated_arguments[1] !== missing && calculated_arguments[1] !== any_object
        if calculated_arguments[2] !== missing && calculated_arguments[2] !== any_object
            ...
            return [calculated_arguments[1], calculated_arguments[1] - value]
        end
    elseif calculated_arguments[2] !== missing && calculated_arguments[2] !== any_object
        return [value + calculated_arguments[2], calculated_arguments[2]]
    end
