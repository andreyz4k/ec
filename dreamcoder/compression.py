...
def stitchInduce(
    ...
    def grammar_from_json(grammar_json: dict) -> Grammar:
        ...
        grammar = Grammar(
            ...
            continuationType=(
                Type.fromjson(grammar["continuationType"])
                if "continuationType" in grammar
                else None
            ),
        )
        return grammar
    ...
    rewritten_progs = stitch_core.rewrite(
        abstractions=compress_result.abstractions, **stitch_kwargs
    )
    name_mapping = stitch_core.name_mapping_dreamcoder(
        dreamcoder_json
    ) + stitch_core.name_mapping_stitch(compress_result.json)
    rewritten_dc = stitch_core.stitch_to_dreamcoder(
    ...
    new_frontiers = [
        new_grammar.rescoreFrontier(frontier)
        for frontier in task_to_unscored_frontier.values()
    ]
