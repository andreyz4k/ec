...
let serialize_frontier f =
  let j : Yojson.Basic.t =
    `Assoc
      [
        ("request", serialize_type f.request);
        ( "programs",
          `List
            (f.programs
            |> List.map ~f:(fun (p, l) ->
                   `Assoc
                     [ ("program", `String (string_of_program p)); ("logLikelihood", `Float l) ]))
        );
      ]
  in
  j
...
let violates_symmetry f a n =
  if not (is_base_primitive f) then false
  else
    let a = application_function a in
    if not (is_base_primitive a) then false
    else
...
let violates_commutative f x y = match f with "eq?" | "+" -> compare_program x y > 0 | _ -> false

(* Best first enumeration *)
let primitive_unknown t g = Primitive (t, "??", ref g |> magical)

type turn = A of tp | L | R

type best_first_state = {
  skeleton : program;
  context : tContext;
  path : turn list;
  cost : float;
...
let path_environment p = p |> List.filter_map ~f:(function A t -> Some t | _ -> None) |> List.rev
...
let string_of_state { skeleton; context; path; cost; _ } =
  ...
  in
  Printf.sprintf "{cost=%f; skeleton=%s; path=%s; env=[%s];}" cost (string_of_program skeleton)
    (path |> List.map ~f:string_of_turn |> join ~separator:" ")
    (path |> path_environment |> List.map ~f:string_of_type |> join ~separator:",")

let state_finished { path; skeleton; _ } =
  match skeleton with Primitive (_, "??", _) -> false | _ -> List.is_empty path
...
let rec follow_path e path =
  ...
  | Abstraction body, A _ :: p' -> follow_path body p'
  | Primitive (_, "??", _), [] -> e
  | _ -> assert false
...
let rec modify_skeleton e q path =
  ...
  | Abstraction body, A _ :: p' -> Abstraction (modify_skeleton body q p')
  | Primitive (_, "??", _), [] -> q
  | _ -> assert false
...
let state_violates_symmetry { skeleton; _ } =
  let rec r = function
    ...
    | Apply (f, x) ->
        ...
        r f || List.exists a ~f:r || List.existsi a ~f:(fun n x' -> violates_symmetry f x' n)
    | _ -> false
  in
  r skeleton
...
let state_successors ~maxFreeParameters cg state =
  ...
  | _ ->
      ...
      |> List.filter ~f:(fun new_state ->
             (not (state_violates_symmetry new_state))
             && new_state.free_parameters <= maxFreeParameters)

let best_first_enumeration ?(lower_bound = None) ?(upper_bound = None) ?(frontier_size = 150)
    ~maxFreeParameters (cg : contextual_grammar) (request : tp) =
  let lower_bound = match lower_bound with None -> -1.0 | Some lb -> lb in
  let upper_bound = match upper_bound with None -> 9999.0 | Some ub -> ub in
  ...
  while Heap.length pq > 0 && Heap.length pq < frontier_size do
    ...
    |> List.iter ~f:(fun child ->
           ...
           else if child.cost < upper_bound then Heap.add pq child
           else ())
  done;

  (!completed, Heap.to_list pq)
...
let rec enumerate_programs' (cg : contextual_grammar) (g : grammar) (context : tContext)
    (request : tp) (environment : tp list) (lower_bound : float) (upper_bound : float)
    ?(maximumDepth = 9999) ?((* Symmetry breaking *)
                             parent = None)
    ?((* We sometimes bound the number of free parameters *)
      maxFreeParameters = 99) (callBack : program -> tContext -> float -> int -> unit) : unit =
  ...
  if enumeration_timed_out () || maximumDepth < 1 || Float.( < ) upper_bound 0.0 then ()
  else
    match request with
    | TCon ("->", [ argument_type; return_type ], _) ->
        let newEnvironment = argument_type :: environment in
        enumerate_programs' ~maximumDepth ~parent:None cg g context return_type newEnvironment
          lower_bound upper_bound ~maxFreeParameters (fun body newContext ll fp ->
            callBack (Abstraction body) newContext ll fp)
    | _ ->
        (* not trying to enumerate functions *)
        let candidates = unifying_expressions g environment request context in
        candidates
        |> List.iter ~f:(fun (candidate, argument_types, context, ll) ->
               let mdl = 0. -. ll in
               if
                 Float.( >= ) mdl upper_bound
                 ||
                 match parent with None -> false | Some (p, j) -> violates_symmetry p candidate j
               then ()
               else
                 ...
                 if fp > maxFreeParameters then ()
                   ...
                   let argument_requests =
                     match candidate with
                     | Index _ ->
                         argument_types |> List.map ~f:(fun at -> (at, cg.variable_context))
                     | _ ->
                         List.Assoc.find_exn cg.contextual_library candidate ~equal:program_equal
                         |> List.zip_exn argument_types
                   in
                   enumerate_contextual_applications ~maximumDepth:(maximumDepth - 1)
                     ~maxFreeParameters:(maxFreeParameters - fp) cg context environment
                     argument_requests candidate (lower_bound +. ll) (upper_bound +. ll)
                     (fun p k al fp' -> callBack p k (ll +. al) (fp + fp')))

and enumerate_contextual_applications ?(maximumDepth = 9999) (cg : contextual_grammar)
    (context : tContext) (environment : tp list) (argument_requests : (tp * grammar) list)
    (f : program) ?(originalFunction = f) ?(argumentIndex = 0) ?(maxFreeParameters = 99)
  ...
  if enumeration_timed_out () || maximumDepth < 1 || Float.( < ) upper_bound 0.0 then ()
    ...
    | (first_argument, first_grammar) :: later_arguments ->
        ...
        enumerate_programs' ~maximumDepth ~maxFreeParameters
          ...
          cg first_grammar context first_argument environment 0. upper_bound
          (fun a k ll fp ->
            let a = Apply (f, a) in
            enumerate_contextual_applications ~originalFunction ~argumentIndex:(argumentIndex + 1)
              ~maximumDepth ~maxFreeParameters:(maxFreeParameters - fp) cg k environment
              later_arguments a (lower_bound +. ll) (upper_bound +. ll) (fun a k a_ll fp' ->
                callBack a k (a_ll +. ll) (fp + fp')))
...
let dfs_around_skeleton cg ~maxFreeParameters ~lower_bound ~upper_bound state k =
  let rec free = function
    ...
    | FreeVar _ -> false
    | Const _ -> false
    | LetClause (_, _, d, b) -> free b || free d
  ...
  let rec around e abstraction_depth ?(parent = None) =
    (* context abstraction_depth l u mfp k *)
    match e with
    | Abstraction body ->
        let around_body = around ~parent:None body (1 + abstraction_depth) in
        fun context l u mfp k ->
          around_body context l u mfp (fun body newContext ll fp ->
              k (Abstraction body) newContext ll fp)
    | Apply (f, x) when free x && not (free f) ->
        let around_argument = around ~parent:(Some (parent_index e)) x abstraction_depth in
        fun context l u mfp k ->
          around_argument context l u mfp (fun x' newContext ll fp ->
              k (Apply (f, x')) newContext ll fp)
    | Apply (f, x) when free f && free x ->
        let around_argument = around ~parent:(Some (parent_index e)) x abstraction_depth in
        let around_function = around ~parent:None f abstraction_depth in
        fun context l u mfp k ->
          around_function context 0. u mfp (fun f' context f_ll fp ->
              around_argument context (l +. f_ll) (u +. f_ll) (mfp - fp) (fun x' context x_ll fp' ->
    ...
    | Primitive (t, "??", g) ->
        ...
        fun context l u mfp k ->
          ...
          enumerate_programs' ~parent cg g context t environment l u ~maxFreeParameters:mfp k
    | _ -> assert false
  in

  around ~parent:None state.skeleton 0 state.context (lower_bound -. state.cost)
    (upper_bound -. state.cost) (maxFreeParameters - state.free_parameters) (fun e context ll _ ->
      k e context (ll -. state.cost))

let shatter_factor = ref 10

(* Putting depth first and best first gives us a parallel strategy for enumeration *)
let multicore_enumeration ?(extraQuiet = false) ?(final = fun () -> []) ?(cores = 1)
    ?(shatter = None) cg request lb ub ~maxFreeParameters k =
  let shatter =
    match (shatter, cores) with Some s, _ -> s | None, 1 -> 1 | None, c -> !shatter_factor * c
  in

  let finished, fringe =
    best_first_enumeration ~lower_bound:(Some lb) ~upper_bound:(Some ub) ~frontier_size:shatter
      ~maxFreeParameters cg request
  in
  ...
  let continuation s =
    dfs_around_skeleton cg ~lower_bound:lb ~upper_bound:ub ~maxFreeParameters s strip_context
  in
...
let enumerate_programs ?(extraQuiet = false) ?(maxFreeParameters = 0) ?(final = fun () -> [])
    ?(nc = 1) cg request lb ub k =
  ...
  let k' =
    if definitely_recursive then fun p l ->
      let p' =
        match p with
        | Abstraction body ->
            if variable_is_bound ~height:0 body then
              ...
              | 2 ->
                  Abstraction
                    (Abstraction (Apply (Apply (Apply (primitive_recursion2, Index 1), Index 0), p)))
  ...
  multicore_enumeration ~extraQuiet ~maxFreeParameters ~final ~cores:nc g' request' lb ub k'
  ...
  let g =
    primitive_grammar
      [
        ...
        primitive0;
        primitive_recursion;
        primitive_recursion2;
      ]
  in
  let request = tlist tint @> tint @> tlist tint in
  enumerate_programs (g |> make_dummy_contextual) request 0. 15. (fun p l ->
...
let test_best_enumeration () =
  ...
  let frontier = ref [] in
  let k p l =
    frontier := (string_of_program p, l) :: !frontier;
    assert (number_of_free_parameters p <= mfp);
    Printf.printf "%s\t%d\n" (string_of_program p) (number_of_free_parameters p)
  in
  (* let open Sys in *)
  enumerate_programs ~maxFreeParameters:mfp
    ~final:(fun () -> List.take !frontier 5)
    ~nc:((Sys.get_argv ()).(1) |> Int.of_string)
    (g |> make_dummy_contextual) request 0.
    ((Sys.get_argv ()).(2) |> Float.of_string)
    k

module EnumerationRequest = struct
  type t = tp * tp list [@@deriving compare, hash, sexp_of]
  ...
  let my_request (r : t) =
    let request, _environment = r in
    request

  let my_environment (r : t) =
    let _request, environment = r in
    environment
  ...
  let substitute k (r, e) : t =
    (applyContext k r |> snd, e |> List.map ~f:(fun t -> applyContext k t |> snd))
end

let make_enumeration_table () = Hashtbl.create (module EnumerationRequest)

let dynamic_programming_enumeration ?(factor = 2) ~lower_bound ~upper_bound (g : grammar) (t : tp)
    (callback : program -> float -> unit) : unit =
  ...
  let rec populate (request : EnumerationRequest.t) cost
    ...
    if cost < 0 then ()
      ...
      | TCon ("->", [ t1; t2 ], _) ->
          populate
            (t2, t1 :: EnumerationRequest.my_environment request)
            cost
      ...
      | _ -> (
            ...
            Hashtbl.find_or_add table request ~default:(fun () ->
                let candidates =
                  ...
                  |> List.map ~f:(fun (p, arguments, k, l) ->
                         ...
                         let environment =
                           EnumerationRequest.my_environment request
                           |> List.map ~f:(fun t -> applyContext k t |> snd)
                         in
                         let request' =
                           EnumerationRequest.my_request request |> applyContext k |> snd
                         in
                         (p, (request', environment), k, arguments, cost))
                in
                (candidates, Array.create ~len:(scale upper_bound + 1) None))
          in
          ...
          | None ->
              let x = ref [] in
              candidates
              |> List.iter ~f:(fun (f, (request', environment'), _k, arguments, my_cost) ->
                     let cost' = cost - my_cost in
                     applications (request', environment') cost' f arguments (fun p ks ->
                         x := (p, ks) :: !x));
              vector.(cost) <- Some !x;
  ...
  and applications request cost f arguments callback =
    ...
    if cost < 0 then ()
      ...
      | 0, _ :: _ -> ()
      | _, [ t1 ] ->
          populate (t1, environment) cost (fun a1 new_a1_request ->
      ...
      | _, t1 :: ts ->
          0 -- cost
          |> List.iter ~f:(fun c1 ->
                 populate (t1, environment) c1 (fun a1 new_a1_request ->
                     let k =
                       EnumerationRequest.request_unifier request (t1, environment) new_a1_request
                     in
                     let returned_request = EnumerationRequest.substitute k request in
                     let f' = Apply (f, a1) in
  ...
  let rec loop c =
    if enumeration_timed_out () || c >= scale upper_bound then ()
      ...
      populate (t, []) c (fun p _ -> callback p l);
      loop (c + 1))
  in
