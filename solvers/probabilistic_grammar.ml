         ...
         |> List.map
              ~f:(fun
                ...
                let arguments =
                  arguments
                  |> List.map ~f:(fun ar ->
                         Printf.sprintf "%dx<%d>" ar.number_of_lambdas ar.nonterminal)
                  |> join ~separator:" "
                in
             ...
             |> List.map ~f:(fun production ->
                    {
                      ...
                      production_constructor =
                        production |> member "constructor" |> to_string |> parse_program
                        |> safe_get_some "Error parsing: PCFG constructor";
                      production_arguments =
                        production |> member "arguments" |> to_list
                        |> List.map ~f:(fun ar ->
                               {
                                 number_of_lambdas = ar |> member "n_lambda" |> to_int;
...
let rec likelihood_PCFG (g : probabilistic_grammar) ?(nt = g.start_symbol) (p : program) : float =
  let f, arguments = application_parse p in
  let arguments = arguments |> List.map ~f:recursively_get_abstraction_body in

  g.productions.(nt) |> Array.to_list
  |> List.filter_map
       ~f:(fun { production_probability; production_constructor; production_arguments } ->
         if program_equal production_constructor f then
           Some
             (List.fold2_exn production_arguments arguments ~init:production_probability
                ~f:(fun likelihood { nonterminal; _ } argument ->
                  likelihood +. likelihood_PCFG g ~nt:nonterminal argument))
         else None)
  |> maximum_by ~cmp:(fun x y -> if Float.( > ) x y then 1 else -1)

let rec bounded_recursive_enumeration ~lower_bound ~upper_bound (g : probabilistic_grammar) ~nt
    (callback : program -> float -> unit) : unit =
  (* Enumerates programs satisfying: lowerBound <= MDL < upperBound *)
  if enumeration_timed_out () || Float.( < ) upper_bound 0.0 then ()
  else
    g.productions.(nt)
    |> Array.iter
         ~f:(fun
           ...
           ->
           if Float.( >= ) (0. -. lp) upper_bound then ()
           else
             bounded_recursive_enumeration_application ~lower_bound:(lower_bound +. lp)
               ~upper_bound:(upper_bound +. lp) g k arguments (fun p argument_likelihood ->
                 callback p (argument_likelihood +. lp)))

and bounded_recursive_enumeration_application ~lower_bound ~upper_bound (g : probabilistic_grammar)
    (f : program) (arguments : rule_argument list) (callback : program -> float -> unit) =
  match arguments with
  | [] -> if Float.( <= ) lower_bound 0. && Float.( < ) 0. upper_bound then callback f 0. else ()
  | { number_of_lambdas; nonterminal } :: later_arguments ->
      bounded_recursive_enumeration ~lower_bound:0. ~upper_bound g ~nt:nonterminal
        (fun first_argument first_argument_likelihood ->
          ...
          bounded_recursive_enumeration_application
            ...
            ~upper_bound:(upper_bound +. first_argument_likelihood) g f later_arguments
...
let bounded_recursive_enumeration ~lower_bound ~upper_bound (g : probabilistic_grammar)
    (callback : program -> float -> unit) : unit =
  bounded_recursive_enumeration ~lower_bound ~upper_bound g ~nt:g.start_symbol callback

let bottom_up_enumeration ?(factor = 10) ~lower_bound ~upper_bound (g : probabilistic_grammar)
    (callback : program -> float -> unit) : unit =
  let factor = Float.of_int factor in
  let scale f = Int.of_float ((f *. factor) +. 0.5) in
  let table =
    g.productions |> Array.map ~f:(fun _ -> Array.create ~len:(scale upper_bound + 1) None)
  in
  let productions = g.productions |> Array.map ~f:Array.to_list in
  ...
  and applications c f arguments =
    if c < 0 then []
      ...
      | ( _,
          [
            { nonterminal = a1; number_of_lambdas = l1 };
            { nonterminal = a2; number_of_lambdas = l2 };
          ] ) ->
          0 -- c
          |> List.concat_map ~f:(fun c1 ->
                 populate a1 c1
                 |> List.concat_map ~f:(fun a1 ->
                        let a1 = wrap_with_abstractions l1 a1 in
                        populate a2 (c - c1)
                        |> List.map ~f:(fun a2 ->
      ...
      | ( _,
          [
            { nonterminal = a1; number_of_lambdas = l1 };
            { nonterminal = a2; number_of_lambdas = l2 };
            { nonterminal = a3; number_of_lambdas = l3 };
          ] ) ->
          0 -- c
          |> List.concat_map ~f:(fun c1 ->
                 populate a1 c1
                 |> List.concat_map ~f:(fun a1 ->
                        let a1 = wrap_with_abstractions l1 a1 in
                        0 -- (c - c1)
                        |> List.concat_map ~f:(fun c2 ->
                               populate a2 c2
                               |> List.concat_map ~f:(fun a2 ->
                                      let a2 = wrap_with_abstractions l2 a2 in
                                      populate a3 (c - c1 - c2)
                                      |> List.map ~f:(fun a3 ->
      ...
      | ( _,
          [
            ...
            { nonterminal = a2; number_of_lambdas = l2 };
            { nonterminal = a3; number_of_lambdas = l3 };
            { nonterminal = a4; number_of_lambdas = l4 };
          ] ) ->
          0 -- c
          |> List.concat_map ~f:(fun c1 ->
                 populate a1 c1
                 |> List.concat_map ~f:(fun a1 ->
                        ...
                        |> List.concat_map ~f:(fun c2 ->
                               ...
                               |> List.concat_map ~f:(fun a2 ->
                                      let a2 = wrap_with_abstractions l2 a2 in
                                      0 -- (c - c1 - c2)
                                      |> List.concat_map ~f:(fun c3 ->
                                             populate a3 c3
                                             |> List.concat_map ~f:(fun a3 ->
                                                    let a3 = wrap_with_abstractions l3 a3 in
                                                    populate a4 (c - c1 - c2 - c3)
                                                    |> List.map ~f:(fun a4 ->
                                                           let a4 = wrap_with_abstractions l4 a4 in
                                                           Apply
                                                             ( Apply (Apply (Apply (f, a1), a2), a3),
                                                               a4 ))))))))
      | _ -> raise (Failure "kind arity >4")
  in
  ...
  let rec loop c =
    if enumeration_timed_out () || c >= scale upper_bound then ()
      ...
      |> List.iter ~f:(fun p -> callback (wrap_with_abstractions g.number_of_arguments p) l);
      loop (c + 1))
  in
