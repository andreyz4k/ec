...
(* open Vs *)
open Differentiation [@@warning "-33"]
open TikZ [@@warning "-33"]
open Utils
open Type
open Program [@@warning "-33"]
open Enumeration
open Task
open Grammar
open FastType [@@warning "-33"]
...
let load_problems channel =
  ...
  let unrolled_grammar = try Some (j |> member "PCFG" |> deserialize_PCFG) with _ -> None in
           ...
           let task =
             (try
              ...
              with _ -> supervised_task)
               ~timeout name task_type examples
           in
  ...
  let _ : unit = try shatter_factor := j |> member "shatter" |> to_int with _ -> () in
...
let run_solver () : unit =
  ...
  let tf, g, unrolled, lowerBound, upperBound, budgetIncrement, mfp, nc, timeout, verbose =
    load_problems Stdlib.stdin
  in
  let _quick_tasks =
    tf
    |> List.map ~f:(fun (t, k) ->
           ({ name = t.name; task_type = t.task_type; log_likelihood = (fun _ -> log 0.) }, k))
  in
  let _slow_tasks =
    tf
    |> List.map ~f:(fun (t, k) ->
           ( {
               ...
               log_likelihood =
                 (fun p ->
                   let _ : float = t.log_likelihood p in
                   log 0.);
             },
             k ))
  in
  ...
  let backend =
    ...
    | None ->
        let traditional_backend lowerBound upperBound ~final =
          enumerate_programs ~final g (List.hd_exn tf |> fst).task_type lowerBound upperBound
            ~maxFreeParameters:mfp ~nc
        in
        traditional_backend
    | Some _unrolled ->
        let new_backend lower_bound upper_bound ~final continuation =
          ...
          dynamic_programming_enumeration ~factor:2 ~lower_bound ~upper_bound g.variable_context
            (List.hd_exn tf |> fst).task_type (* unrolled *) (fun p l -> continuation p l);
          [ final () ]
        in
        new_backend
  in
  ...
  let solutions, number_enumerated =
    enumerate_for_tasks backend ~lowerBound ~upperBound ~budgetIncrement ~verbose ~timeout tf ~nc
  in
